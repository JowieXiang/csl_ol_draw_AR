...
public class ProceduralUtilsEgypt
{
    ...
    public List<List<Vector3>> GetCoordinates(string wsString)
    {
        ...
        for (int i = 0; i < json["features"].Count; i++)
        {
            ...
            drawObjList.Add(JsonMapper.ToObject("{\"coordinates\":[],\"height\":1000}"));
            //since the JSON received from ws has a superfluous item, we iterate until count-1
            int pCount = json["features"][i]["geometry"]["coordinates"][0][0].Count - 1;
            JsonData[] arr = new JsonData[pCount];
            for (int j = 0; j < pCount; j++)
            {
                arr[j] = JsonMapper.ToObject("{\"lat\":" + json["features"][i]["geometry"]["coordinates"][0][0][j][0] + ",\"lon\":" + json["features"][i]["geometry"]["coordinates"][0][0][j][1] + "}");

                Coordinates[i].Add(json["features"][i]["geometry"]["coordinates"][0][0][j]);
            }
            foreach (JsonData item in arr)
            {
                drawObjList[i]["coordinates"].Add(item);
            }
            if (json["features"][i]["properties"].Keys.Contains("floors"))
            {
                drawObjList[i]["height"] = json["features"][i]["properties"]["floors"];
            }
        }
    ...
    public List<List<Vector3>> GetSideVertices(List<Vector3> basePoints, float height)
    {
        ...
        for (int i = 0; i < newBasePoints.Count - 1; i++)
        {
            ...
            Vector3 h1 = new Vector3(newBasePoints[i].x, height*4, newBasePoints[i].z);
            Vector3 h2 = new Vector3(newBasePoints[i + 1].x, height*4, newBasePoints[i + 1].z);
    ...
    public List<Vector3> GetCap(List<Vector3> basePoints, float height)
    {
        ...
        {
            capVertices.Add(new Vector3(basePoints[i].x, height*4, basePoints[i].z));
        }
        return capVertices;
    }
